...
namespace Nutaku.Unity
{
    ...
    static class OAuthUtils
    {
            ...
            IEnumerable<KeyValuePair<string, string>> additionalOAuthHeaderParams = null)
        {
            ...
            oauthHeaderParams.Add("oauth_consumer_key", oauthParams.consumerKey);
            oauthHeaderParams.Add("oauth_nonce", System.Guid.NewGuid().ToString());
            oauthHeaderParams.Add("oauth_signature_method", "HMAC-SHA1");
            oauthHeaderParams.Add("oauth_timestamp",
                Convert.ToInt64((DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds).ToString());
            oauthHeaderParams.Add("oauth_version", "1.0");
